...
# For use if you have no BRATS installation or wish to use Python for this rather.
...
def main():
    """
    For use if you have no BRATS installation or wish to use Python for spectral index calculation rather.
    """
        ...
        for k in range(0, len(im_data[0][0,:])): # ra
            try:
                ...
                for l in range(0, len(im_data)):
                    ...
                    if im_data[l][j,k] >= sigma_thresh * (std_dev): # sigma threshold
    ...
    try:
        ...
        field_name = os.path.basename(files[0])[:-5]
